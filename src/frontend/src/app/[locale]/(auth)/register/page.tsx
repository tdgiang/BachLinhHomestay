  ...
  return (
    <div className="w-full max-w-md">
      <div className="flex items-center gap-2 mb-8 justify-center">
        ...
        <span className="font-bold text-lg" style={{ color: 'var(--color-text-primary)' }}>Ocean Blue</span>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-8" style={{ borderColor: 'var(--color-border)' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-1.5">
            ...
            {errors.fullName && <p className="text-xs" style={{ color: 'var(--color-danger)' }}>{errors.fullName.message}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="login">{t('emailOrPhone')}</Label>
            <Input id="login" placeholder="email@example.com hoặc 09xxxxxxxx" {...register('login')} />
            {errors.login && <p className="text-xs" style={{ color: 'var(--color-danger)' }}>{errors.login.message}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            />
            {errors.confirmPassword && <p className="text-xs" style={{ color: 'var(--color-danger)' }}>{errors.confirmPassword.message}</p>}
          </div>

          {error && (
            <div className="text-sm rounded-lg px-3 py-2.5" style={{ background: '#FEF2F2', color: 'var(--color-danger)' }}>
              {error}
            </div>
          )}
          ...
          >
            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : t('registerButton')}
          </Button>
        </form>

        <p className="text-sm text-center mt-5" style={{ color: 'var(--color-text-secondary)' }}>
          {t('hasAccount')}{' '}
